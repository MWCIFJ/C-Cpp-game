...
void InitInterface()
{
	...
	for (int i = COL; i < COL + 10; i++)//列数：游戏旁支~游戏旁支+10
	{
	...
	for (int shape = 0; shape < 7; shape++) //7种形状
	{
		...
		{//通过按space键来通过顺时针旋转，进行更改形态
			...
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					block[shape][form + 1].space[i][j] = temp[3 - j][i];//切换的指令
				}
			}
		}
	}
...
void color(int c)
{
	...
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), c); //颜色设置 就是在这个函数里进行数字匹配颜色的
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			if (block[shape][form].space[i][j] == 1) //如果该位置有方块
			{
				...
				printf("■"); //输出方块（给InitBlockInfo()输出来）
			}
		}
	}
}
...
int JudeFunc()
{
	...
	for (int i = ROW - 2; i > 4; i--)
	{
		...
		for (int j = 1; j < COL - 1; j++)//j范围：行头~行尾
		{
			sum += face.data[i][j]; //统计第i行的方块个数
		}
		if (sum == 0) //该行没有方块，无需再判断其上的层次（无需再继续判断是否得分）
			break; //跳出循环
		if (sum == COL - 2) //该行的方块可以连成一整行，可得分
		{
			grade += 10; //满一行加10分
			color(7); //颜色设置为白色
			CursorJump(2 * COL + 4, ROW - 3); //光标【跳转】到显示当前分数的位置
			printf("当前分数：%d", grade); //更新当前分数
			for (int j = 1; j < COL - 1; j++) //清除得分行的方块
			{
				face.data[i][j] = 0; //该位置得分后被清除，标记为无方块
				CursorJump(2 * j, i); //光标跳转到该位置，配合循环以达到遍历逐个坐标的效果
				printf("  "); //打印空格覆盖（两个空格），以实现消除的效果
			}
			...
			for (int m = i; m > 1; m--)
			{
				...
				for (int n = 1; n < COL - 1; n++)
				{
					...
					if (face.data[m][n] == 1) //上一行移下来的是方块，打印方块
					{
						...
						printf("■"); //打印方块（本质上就是上一行的方块在这一行闪现输出）
					}
					else //上一行移下来的是空格，打印空格
					{
						CursorJump(2 * n, m); //光标跳转到该位置
						printf("  "); //打印空格（两个空格）（上一行是空格的，这行还是空格）
					}
				}
	...
	for (int j = 1; j < COL - 1; j++)//框内的行遍历
	{
		if (face.data[1][j] == 1) //顶层有方块存在（以第1行为顶层，不是第0行）
		{
			...
			{
				printf("与最高记录%d分持平，加油再创佳绩", grade);
			}
			else
			{
...
void StartGame()
{
	int shape = rand() % 7, form = rand() % 4; //随机获取方块的【形状】和【形态】
	while (1)
	{
		int t = 0;
		int nextShape = rand() % 7, nextForm = rand() % 4; //随机获取下一个方块的【形状】和【形态】
		...
		while (1)
		{
			...
			while (--t)	
			{
			 /*说实话，这种循环的方式比用sleep()函数低级10000倍也不为过
			 它过分占用性能了，一个简单的小游戏的CPU性能消耗比兰斯10还高
			 这比伪冒泡排序的交换排序还要恶心人*/

				if (kbhit() != 0) //若键盘被敲击，则退出循环
			//这里是规定时间内（实际上是按CPU主频循环执行次数而有着类似帧率的时间）如果什么都不做就会自动下降一格
					break;
			}
			if (t == 0) //键盘未被敲击
			{
				if (IsLegal(shape, form, x, y + 1) == 0) //方块再下落就不合法了（已经到达底部）
				{
					...
					for (int i = 0; i < 4; i++)
					{
						for (int j = 0; j < 4; j++)
						{
							if (block[shape][form].space[i][j] == 1)//让这个在底部的方块有着标签
							{
			...
			else //键盘被敲击
			{
				...
				case 'S':  //暂停
					...
					//pause>nul相比pause，还会不显示“请按任意键继续...”这些字
					break;
				case 'r':
				case 'R': //重新开始
					system("cls"); //清空屏幕
		...
		shape = nextShape;
		form = nextForm; //获取下一个方块的信息
		DrawSpace(nextShape, nextForm, COL + 3, 3); //将右上角的方块信息用空格覆盖
		//不进行空格覆盖会导致旧方块没被覆写成空格，这样新方块就会与之重合
	}
}
