...
struct Player //玩家结构体，并初始化player
{
	...
	int range_exp;
	long int max_exp;
} player = { "鬼畜勇者",50,40,100,100,1,0,0,100 };
...
moren = { "魔人","\0",60 * 8,30 * 8,280 * 8,200 * 8,100 * 8,9,1,1,0 },
...
int main()
{
	...
	int i = 0, j = 0, k = 0;
	char player_name[21] = "chara";
	Sleep(1000);
	printf("--------------------------欢迎来到 AAA 1.0 beta-----------------------\n\n\n");
	//如果想使用外挂，名字请输入：“圣战斗士”。
	Sleep(1000);
	printf("这里是AAA世界! 利萨斯国的Lia公主被魔人绑架了!\n\n\n伟大的勇者啊~拿起你们的武器,营救公主!\n\n\n输入你的名字: (20个字符)\n\n\n");
	scanf_s("%c", player_name);
...
void ChooseWupin() //选择物品 并使用
{
	...
	case 7:
		if (battle) //在战斗中(battle=1),否则(battle=0)不能使用攻击性物品
		{
			if (atom_boom > 0)
			{
				printf("使用手抛式原子弹,敌人HP减少2147483647\n\n\n");
				atom_boom--;
				guai.health -= 2147483647;
				AttackResult();
			}
		}
...
void OrdinaryAct() //正常行为菜单(移动,物品,对话,查看状态,装备,退出游戏)
{
	...
	while (1)
	{
			...
			case 5:
				...
				while (1)
				{
					...
					case 6:
						...
						{
							battle = 1;
							if (strongman_arm)	//持有巨人晶石才能触发BOSS战
							{
								printf("神秘人:\n\n\n 哈哈,年轻人,做的不错,不过...嘿嘿,你上当啦!巨人晶石我要了,公主你也别想带走!\n\n\n");
								guai = moren;	//神秘人就是最终BOSS
								printf("%s扑了过来!\n\n\n", guai.name);
								BattleAct();
							}
		...
		case 5: //装备
			...
			break;
		case 6:
			printf(" 您好，欢迎您玩AAA世界。为了给您更好的游戏体验，本团队时不时会优化本游戏，优化后会尽快发布在网上。关于外挂方面，开启外挂的方式是设定勇者姓名时，输入“圣战斗士 ”（不包括双引号）。由于2.0版本的buy，我们在2.0的基础上进行修改，已修复该bug。并且新增了经验草等有助于升级的道具，希望大家喜欢。在这里要感谢离陌同学，他给了我们许多宝贵的建议，谢谢。\n");
			break;
		case 0:
			printf("确定退出游戏?(Y/N)\n\n\n");
			getchar();
